    ...
    <section className={`workspace ${collapsed ? "sidebar-collapsed" : ""}`}>
      ...
      {section === "billing" && <div className="billing-layout">
        <section className="catalog-panel">
          ...
          <div className="scanner-row"><div className="scanner-input"><span className="scanner-icon"><Search size={21} /></span><input ref={barcodeRef} value={query} onChange={(e) => setQuery(e.target.value)} placeholder={language === "ta" ? "பொருள் பெயர் / barcode தேடுங்கள்..." : "Search product name or barcode..."} aria-label="Search billing products" />{query && <button onClick={() => setQuery("")}><X size={16} /></button>}</div><button className="scan-button" onClick={() => { setBarcodeValue(query); const product = products.find((item) => item.barcode === query.trim()); if (product) { addToCart(product); setQuery(""); } else notify("Type a product name or exact barcode"); }}><Barcode size={18} /> Barcode</button></div>
          ...
          <div className="catalog-heading"><div><h2>{t.catalog}</h2><span>{filteredProducts.length} items available</span></div><div className="catalog-tools"><button className="icon-button"><ListFilter size={18} /></button></div></div>
